...
class ConnectionPool:
    def __init__(self):
        # 每條 SSE 連線一個 asyncio.Queue（信箱），用 set 維護連線池（O(1) 移除）
    ...
    def unregister(self, q: asyncio.Queue) -> None:
        # 連線斷了：把信箱移出集合。discard 不存在也不報錯（斷線和重整可能重複觸發）
        self.connections.discard(q)

    def broadcast(self, event_name: str, data: dict) -> None:
        # 走訪集合，往每個信箱投一份訊息副本
        # set(...) 複製一份再走訪，避免走訪途中有人 unregister 導致集合大小變動
        message = {"event": event_name, "data": data}
        for q in set(self.connections):
