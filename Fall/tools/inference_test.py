...
def camera_worker(camera_id, video_source):
    ...
    if fps <= 0 or np.isnan(fps): fps = 24.0  # 對齊 RTSP 串流原生 24 FPS 影格率
    frame_delay = 1.0 / fps  
    ...
    if camera_id == "Room_301_Bed":
        ...
        try:
            ...
            print(f"📡 [{camera_id}] 已成功啟動 MediaMTX 即時推流中繼 ➔ {output_rtsp_url}")
        except Exception as e:
    ...
    while True:
        ...
        if frame_count % 30 == 0 or cached_results_env is None:
            ...
            if triton_client is not None:
                try:
                    ...
                    print(f"⚠️ [{camera_id}] Triton 引擎異常 ({triton_err})，觸發自動降級機制，改用本地 CPU 推理...")
        ...
        if is_recording_post and post_frame_count >= MAX_POST_FRAMES:
            ...
            def _async_process_video(frames, video_path, snapshot_path, cam_id, num_id, evt_label, f_score, act_conf, is_occluded, prod):
                ...
                try:
                    ...
                    s3_client.upload_file(snapshot_path, bucket_name, s3_snapshot_key)
                    s3_client.upload_file(video_path, bucket_name, s3_video_key)
                    ...
                    if prod is not None:
                        if (act_conf > 0.90 or evt_label == "chair_slip") and not is_occluded:
                            fast_track_payload = {
                                "device_id": num_id, "event_type": evt_label, "clip_path": real_s3_video_url,            
                                "detected_at": datetime.now().isoformat(),  
                                "snapshot_path": real_s3_snapshot_url, "image_filename": real_s3_snapshot_url,
                                "yolo_score": f_score,
                                "vlm_summary": "【緊急通報】邊緣端偵測到意外！已實時同步 10 秒影片與影像至雲端 S3。"
                            }
                            prod.send('processed-reports', value=fast_track_payload)
                            prod.flush()
                            print(f"🚀 [{cam_id}] 10秒影片數據已轟入 processed-reports！")
                        else:
                            vlm_queue_payload = {
                                "device_id": num_id, "event_type": evt_label, "clip_path": real_s3_video_url,
                                "detected_at": datetime.now().isoformat(),
                                "snapshot_path": real_s3_snapshot_url, "image_filename": real_s3_snapshot_url,
                                "yolo_score": f_score,
                                "vlm_summary": "【AI 二審機制】雲端 10 秒影片與快照已上傳，正非同步分析中..."
                            }
                            prod.send('nursing-home-alerts', value=vlm_queue_payload)
                            prod.flush()
                            print(f"🚀 [{cam_id}] 10秒影片數據已進入二審佇列！")
        ...
        # 📤 將畫好骨架與跌倒紅框的影像即時推送到 MediaMTX (cam_out)
        if rtsp_writer_proc and rtsp_writer_proc.stdin:
            try:
                rtsp_writer_proc.stdin.write(annotated_frame.tobytes())
            except Exception:
                pass
...
if __name__ == "__main__":
    camera_channels = {
        "Room_301_Bed": "rtsp://localhost:8554/cam_in",        
        # "Room_302_Bed": os.path.join(PROJECT_ROOT, "test_demo", "test2.mp4"),        
        # "Room_303_Bed": os.path.join(PROJECT_ROOT, "test_demo", "test3.mp4"),
    }
